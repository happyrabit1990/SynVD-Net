...
def DnCNN_infer(test_files_gt, test_files, save_dir, Height, Width):
    ...
    for idx in range(len(test_files)):
        ...
        with torch.no_grad(): # this can save much memory
            denoised_y = (torch.squeeze(INoisy-model(INoisy)).cpu().numpy().clip(0, 1)*255.0).astype(np.uint8)
...
if __name__ == "__main__":
    ...
    DnCNN_infer(test_files_gt, test_files, save_file, Height, Width)
