...
parser = argparse.ArgumentParser(description="SynVDUnet_Test")
...
def Unet_infer(test_files_gt, test_files, save_dir, Height, Width):
...
if __name__ == "__main__":
    ...
    # net = UNet(in_channels=1, output_channels=1, bilinear=opt.bilinear)
    net = UNet(in_ch=1, out_ch=1)
    model = net.to(device)
    model.load_state_dict(torch.load(os.path.join(opt.logdir, 'net-2.pth')))
    ...
    Unet_infer(test_files_gt, test_files, save_file, Height, Width)
